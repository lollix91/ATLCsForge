...
/**
 * A text editor for 'atl' models.
 * <p>
 * This editor has id <code>atl.resource.atl.ui.AtlEditor</code>
 * The editor's context menu has id <code>atl.resource.atl.EditorContext</code>. 
 * The editor's ruler context menu has id
 * <code>atl.resource.atl.EditorRuler</code>.
 * </p>
 */
public class AtlEditor extends org.eclipse.ui.editors.text.TextEditor implements org.eclipse.emf.edit.domain.IEditingDomainProvider, org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener, org.eclipse.emf.common.ui.viewer.IViewerProvider, atl.resource.atl.IAtlResourceProvider, atl.resource.atl.ui.IAtlBracketHandlerProvider, atl.resource.atl.ui.IAtlAnnotationModelProvider {
	
	private atl.resource.atl.ui.AtlHighlighting highlighting;
	private org.eclipse.jface.text.source.projection.ProjectionSupport projectionSupport;
	private atl.resource.atl.ui.AtlCodeFoldingManager codeFoldingManager;
	private atl.resource.atl.ui.AtlBackgroundParsingStrategy bgParsingStrategy = new atl.resource.atl.ui.AtlBackgroundParsingStrategy();
	private java.util.Collection<atl.resource.atl.IAtlBackgroundParsingListener> bgParsingListeners = new java.util.ArrayList<atl.resource.atl.IAtlBackgroundParsingListener>();
	private atl.resource.atl.ui.AtlColorManager colorManager = new atl.resource.atl.ui.AtlColorManager();
	private atl.resource.atl.ui.AtlOutlinePage outlinePage;
	...
	private atl.resource.atl.ui.AtlPropertySheetPage propertySheetPage;
	...
	private atl.resource.atl.ui.IAtlBracketHandler bracketHandler;
	...
	public AtlEditor() {
		super();
		setSourceViewerConfiguration(new atl.resource.atl.ui.AtlSourceViewerConfiguration(this, this, this, colorManager));
	...
	private final class DocumentListener implements org.eclipse.jface.text.IDocumentListener {
		...
		public void documentChanged(org.eclipse.jface.text.DocumentEvent event) {
			bgParsingStrategy.parse(event, getResource(), AtlEditor.this);
		}
	}
	...
	private class ModelResourceChangeListener implements org.eclipse.core.resources.IResourceChangeListener {
		public void resourceChanged(org.eclipse.core.resources.IResourceChangeEvent event) {
			...
			try {
					...
					public boolean visit(org.eclipse.core.resources.IResourceDelta delta) {
						...
						if (deltaKind == org.eclipse.core.resources.IResourceDelta.CHANGED && delta.getFlags() != org.eclipse.core.resources.IResourceDelta.MARKERS) {
							...
							if (changedResource != null) {
								...
								atl.resource.atl.IAtlTextResource currentResource = getResource();
				...
				atl.resource.atl.ui.AtlUIPlugin.logError("Unexpected Error: ", exception);
			}
		}
	}
	...
	public void initializeEditor() {
		...
		setEditorContextMenuId("atl.resource.atl.EditorContext");
		setRulerContextMenuId("atl.resource.atl.EditorRuler");
	}
	...
	public void createPartControl(org.eclipse.swt.widgets.Composite parent) {
		...
		highlighting = new atl.resource.atl.ui.AtlHighlighting(getResource(), viewer, colorManager, this);
		...
		codeFoldingManager = new atl.resource.atl.ui.AtlCodeFoldingManager(viewer, this);
	}
	...
	private void initializeResourceObject(org.eclipse.ui.IEditorInput editorInput) {
		...
		atl.resource.atl.mopp.AtlNature.activate(inputFile.getProject());
		...
		atl.resource.atl.IAtlTextResource loadedResource = (atl.resource.atl.IAtlTextResource) resourceSet.getResource(uri, false);
		if (loadedResource == null) {
			try {
				...
				atl.resource.atl.IAtlTextResource currentResource = this.resource;
				...
				if (demandLoadedResource instanceof atl.resource.atl.IAtlTextResource) {
					setResource((atl.resource.atl.IAtlTextResource) demandLoadedResource);
				} else {
					// the resource was not loaded by an EMFText resource, but some other EMF resource
					atl.resource.atl.ui.AtlUIPlugin.showErrorDialog("No EMFText resource.", "The file '" + uri.lastSegment() + "' of type '" + uri.fileExtension() + "' can not be handled by the AtlEditor.");
					// close this editor because it can not present the resource
					close(false);
				}
			} catch (Exception e) {
				atl.resource.atl.ui.AtlUIPlugin.logError("Exception while loading resource in " + this.getClass().getSimpleName() + ".", e);
			}
		} else {
	...
	private void setResource(atl.resource.atl.IAtlTextResource resource) {
	...
	private Object getOutlinePage() {
		if (outlinePage == null) {
			outlinePage = new atl.resource.atl.ui.AtlOutlinePage(this);
	...
	public org.eclipse.ui.views.properties.IPropertySheetPage getPropertySheetPage() {
		if (propertySheetPage == null) {
			propertySheetPage = new atl.resource.atl.ui.AtlPropertySheetPage();
	...
	public void setCaret(org.eclipse.emf.ecore.EObject element, String text) {
		try {
			...
			org.eclipse.jface.text.source.ISourceViewer viewer = getSourceViewer();
			atl.resource.atl.IAtlTextResource textResource = (atl.resource.atl.IAtlTextResource) element.eResource();
			atl.resource.atl.IAtlLocationMap locationMap = textResource.getLocationMap();
			int destination = locationMap.getCharStart(element);
			int length = locationMap.getCharEnd(element) + 1 - destination;
			
			atl.resource.atl.IAtlTextScanner lexer = getResource().getMetaInformation().createLexer();
			try {
				lexer.setText(viewer.getDocument().get(destination, length));
				atl.resource.atl.IAtlTextToken token = lexer.getNextToken();
			...
			atl.resource.atl.ui.AtlUIPlugin.logError("Exception in setCaret()", e);
		}
	}
	...
	protected org.eclipse.jface.text.source.ISourceViewer createSourceViewer(org.eclipse.swt.widgets.Composite parent, org.eclipse.jface.text.source.IVerticalRuler ruler, int styles) {
		org.eclipse.jface.text.source.ISourceViewer viewer = new org.eclipse.jface.text.source.projection.ProjectionViewer(parent, ruler, getOverviewRuler(), isOverviewRulerVisible(), styles) {
			...
			public void setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
				if (!AtlEditor.this.setSelection(selection, reveal)) {
	...
	public void addBackgroundParsingListener(atl.resource.atl.IAtlBackgroundParsingListener listener) {
		bgParsingListeners.add(listener);
	}
	
	public void notifyBackgroundParsingFinished() {
		for (atl.resource.atl.IAtlBackgroundParsingListener listener : bgParsingListeners) {
			listener.parsingCompleted(getResource());
		}
	}
	
	public atl.resource.atl.ui.IAtlBracketHandler getBracketHandler() {
		return bracketHandler;
	}
	
	public void setBracketHandler(atl.resource.atl.ui.IAtlBracketHandler bracketHandler) {
	...
	private boolean setSelection(org.eclipse.jface.viewers.ISelection selection, boolean reveal) {
		if (selection instanceof org.eclipse.jface.viewers.IStructuredSelection) {
			...
			if (object instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (!(resource instanceof atl.resource.atl.IAtlTextResource)) {
					return false;
				}
				atl.resource.atl.IAtlTextResource textResource = (atl.resource.atl.IAtlTextResource) resource;
				atl.resource.atl.IAtlLocationMap locationMap = textResource.getLocationMap();
				int destination = locationMap.getCharStart(element);
				if (destination < 0) {
					destination = 0;
