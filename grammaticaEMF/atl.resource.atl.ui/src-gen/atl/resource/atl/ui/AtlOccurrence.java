 ...
 */
public class AtlOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "atl.resource.atl.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "atl.resource.atl.ui.occurences.declaration";
	
	private final static atl.resource.atl.ui.AtlPositionHelper positionHelper = new atl.resource.atl.ui.AtlPositionHelper();
	
	private atl.resource.atl.ui.AtlTokenScanner tokenScanner;
	...
	private atl.resource.atl.IAtlTextResource textResource;
	 ...
	 */
	public AtlOccurrence(atl.resource.atl.IAtlTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, atl.resource.atl.ui.AtlTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new atl.resource.atl.mopp.AtlMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(atl.resource.atl.ui.AtlBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, atl.resource.atl.ui.AtlPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, atl.resource.atl.ui.AtlPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, atl.resource.atl.ui.AtlPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == atl.resource.atl.ui.AtlPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(atl.resource.atl.ui.AtlOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(atl.resource.atl.ui.AtlOccurrence.DECLARATION_ANNOTATION_ID);
	}
