...
public class AtlBracketSet {
	...
	private java.util.ArrayList<atl.resource.atl.IAtlBracketPair> bracketPairs;
	 ...
	 */
	private class BracketPair implements atl.resource.atl.IAtlBracketPair {
	 ...
	 */
	private class ClosingListener implements atl.resource.atl.ui.IAtlBracketHandler, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				atl.resource.atl.IAtlBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	 ...
	 */
	public AtlBracketSet(atl.resource.atl.ui.AtlEditor editor, org.eclipse.jface.text.source.ISourceViewer sourceViewer) {
		languageID = new atl.resource.atl.mopp.AtlMetaInformation().getSyntaxName();
		this.bracketPairs = new java.util.ArrayList<atl.resource.atl.IAtlBracketPair>();
	 ...
	 */
	public atl.resource.atl.IAtlBracketPair getBracketPair(String opening, String closing) {
		for (atl.resource.atl.IAtlBracketPair bracketPair : bracketPairs) {
	...
	public atl.resource.atl.IAtlBracketPair getBracketPair(int index) {
		try {
	 ...
	 */
	public boolean setClosingEnabledInside(atl.resource.atl.IAtlBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + atl.resource.atl.ui.AtlPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	 ...
	 */
	public atl.resource.atl.IAtlBracketPair remove(String opening, String closing) {
		for (atl.resource.atl.IAtlBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<atl.resource.atl.IAtlBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, atl.resource.atl.ui.AtlPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, atl.resource.atl.ui.AtlPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
