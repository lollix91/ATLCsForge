 ...
 */
public class AtlHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	
	private final static atl.resource.atl.ui.AtlPositionHelper positionHelper = new atl.resource.atl.ui.AtlPositionHelper();
	...
	private atl.resource.atl.ui.AtlTokenScanner scanner;
	...
	private atl.resource.atl.ui.AtlEditor editor;
	private org.eclipse.jface.text.source.projection.ProjectionViewer projectionViewer;
	private atl.resource.atl.ui.AtlOccurrence occurrence;
	private atl.resource.atl.ui.AtlBracketSet bracketSet;
	 ...
	 */
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, atl.resource.atl.IAtlBackgroundParsingListener {
	 ...
	 * @param editor
	 */
	public AtlHighlighting(atl.resource.atl.IAtlTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, atl.resource.atl.ui.AtlColorManager colorManager, atl.resource.atl.ui.AtlEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = atl.resource.atl.ui.AtlUIPlugin.getDefault().getPreferenceStore();
		this.editor = editor;
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new atl.resource.atl.ui.AtlTokenScanner(textResource, colorManager);
		occurrence = new atl.resource.atl.ui.AtlOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new atl.resource.atl.ui.AtlBracketSet(editor, sourceviewer);
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(atl.resource.atl.ui.AtlPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, atl.resource.atl.ui.AtlPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(atl.resource.atl.ui.AtlEditor editor) {
	...
	private void setBracketHighlighting(org.eclipse.jface.text.IDocument document) {
		...
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, atl.resource.atl.ui.AtlPositionCategory.BRACKET.toString());
	...
	private void removeHighlighting() {
		...
		removeHighlightingCategory(document, atl.resource.atl.ui.AtlPositionCategory.BRACKET.toString());
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(atl.resource.atl.ui.AtlPositionCategory.BRACKET.toString())) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new atl.resource.atl.ui.AtlEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
