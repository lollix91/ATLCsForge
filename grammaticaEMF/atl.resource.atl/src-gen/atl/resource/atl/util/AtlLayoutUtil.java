...
public class AtlLayoutUtil {
	...
	public void transferLayoutInformationToModel(org.eclipse.emf.ecore.EObject element) {
		...
		for (java.util.Iterator<atl.resource.atl.mopp.AtlLayoutInformation> i = layoutInformationAdapter.getLayoutInformations().iterator(); i.hasNext(); ) {
			atl.resource.atl.mopp.AtlLayoutInformation layoutInformation = i.next();
	...
	public void transferLayoutInformationFromModel(org.eclipse.emf.ecore.EObject element) {
		...
		if (layoutReference != null) {
			...
			for (java.util.Iterator<org.eclipse.emf.ecore.EObject> i = list.iterator(); i.hasNext(); ) {
				...
				atl.resource.atl.mopp.AtlLayoutInformation layoutInformation = createLayoutInformation(layoutModelElement);
	...
	public org.eclipse.emf.ecore.EObject createLayoutInformationModelElement(atl.resource.atl.mopp.AtlLayoutInformation layoutInformation, org.eclipse.emf.ecore.EPackage layoutPackage) {
		...
		atl.resource.atl.grammar.AtlSyntaxElement syntaxElement = layoutInformation.getSyntaxElement();
		...
		layoutInformationModelElement.eSet(layoutInformationEClass.getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME), atl.resource.atl.grammar.AtlGrammarInformationProvider.getSyntaxElementID(syntaxElement));
		return layoutInformationModelElement;
	}
	
	public atl.resource.atl.mopp.AtlLayoutInformation createLayoutInformation(org.eclipse.emf.ecore.EObject layoutInformationModelElement) {
		Object object = null;
		...
		atl.resource.atl.grammar.AtlSyntaxElement syntaxElement = atl.resource.atl.grammar.AtlGrammarInformationProvider.getSyntaxElementByID((String) layoutInformationModelElement.eGet(layoutInformationModelElement.eClass().getEStructuralFeature(SYNTAX_ELEMENT_ID_EATTRIBUTE_NAME)));
		if (objectFeature != null) {
			object = layoutInformationModelElement.eGet(objectFeature);
		} else if (syntaxElement instanceof atl.resource.atl.grammar.AtlPlaceholder) {
			atl.resource.atl.grammar.AtlPlaceholder placeholder = (atl.resource.atl.grammar.AtlPlaceholder) syntaxElement;
			atl.resource.atl.IAtlTokenResolver tokenResolver = new atl.resource.atl.mopp.AtlTokenResolverFactory().createTokenResolver(placeholder.getTokenName());
			atl.resource.atl.IAtlTokenResolveResult result = new atl.resource.atl.mopp.AtlTokenResolveResult();
			tokenResolver.resolve(visibleTokenText, placeholder.getFeature(), result);
			object = result.getResolvedToken();
		}
		return new atl.resource.atl.mopp.AtlLayoutInformation(syntaxElement, object, startOffset, hiddenTokenText, visibleTokenText);
	}
	
	public atl.resource.atl.mopp.AtlLayoutInformationAdapter getLayoutInformationAdapter(org.eclipse.emf.ecore.EObject element) {
