 ...
 * contains <code>atl.resource.atl.IAtlTextDiagnostic</code>s it uses the more
 * precise information of this extended diagnostic type.
 */
public class AtlMarkerHelper {
	
	/**
	 * The extension id of the custom marker type that is used by this text resource.
	 */
	public static final String MARKER_TYPE = atl.resource.atl.mopp.AtlPlugin.PLUGIN_ID + ".problem";
	...
	private static class MarkerCommandQueue {
		...
		private java.util.List<atl.resource.atl.IAtlCommand<Object>> commands = new java.util.ArrayList<atl.resource.atl.IAtlCommand<Object>>();
		
		private MutexRule schedulingRule = new MutexRule();
		
		public void addCommand(atl.resource.atl.IAtlCommand<Object> command) {
		...
		public void runCommands() {
			java.util.List<atl.resource.atl.IAtlCommand<Object>> commandsToProcess = new java.util.ArrayList<atl.resource.atl.IAtlCommand<Object>>();
			synchronized(commands) {
				commandsToProcess.addAll(commands);
				commands.clear();
			}
			for (atl.resource.atl.IAtlCommand<Object> command : commandsToProcess) {
	 ...
	 */
	public void mark(org.eclipse.emf.ecore.resource.Resource resource, atl.resource.atl.IAtlTextDiagnostic diagnostic) {
	...
	protected void createMarkerFromDiagnostic(final org.eclipse.core.resources.IFile file, final atl.resource.atl.IAtlTextDiagnostic diagnostic) {
		final atl.resource.atl.IAtlProblem problem = diagnostic.getProblem();
		atl.resource.atl.AtlEProblemType problemType = problem.getType();
		final String markerID = getMarkerID(problemType);
		COMMAND_QUEUE.addCommand(new atl.resource.atl.IAtlCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					if (problem.getSeverity() == atl.resource.atl.AtlEProblemSeverity.ERROR) {
					...
					atl.resource.atl.IAtlTextDiagnostic textDiagnostic = (atl.resource.atl.IAtlTextDiagnostic) diagnostic;
					...
					if (diagnostic instanceof atl.resource.atl.mopp.AtlResource.ElementBasedTextDiagnostic) {
						org.eclipse.emf.ecore.EObject element = ((atl.resource.atl.mopp.AtlResource.ElementBasedTextDiagnostic) diagnostic).getElement();
					...
					java.util.Collection<atl.resource.atl.IAtlQuickFix> quickFixes = textDiagnostic.getProblem().getQuickFixes();
					...
					if (!sourceIDs.isEmpty()) {
						marker.setAttribute(org.eclipse.core.resources.IMarker.SOURCE_ID, atl.resource.atl.util.AtlStringUtil.explode(sourceIDs, "|"));
					}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource) {
		for (atl.resource.atl.AtlEProblemType nextType : atl.resource.atl.AtlEProblemType.values()) {
	 ...
	 */
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, atl.resource.atl.AtlEProblemType problemType) {
		final org.eclipse.core.resources.IFile file = getFile(resource);
		if (file == null) {
			return;
		}
	...
	public void unmark(org.eclipse.emf.ecore.resource.Resource resource, final org.eclipse.emf.ecore.EObject causingObject) {
		...
		final String markerID = getMarkerID(atl.resource.atl.AtlEProblemType.UNKNOWN);
	 ...
	 * given type.
	 */
	public String getMarkerID(atl.resource.atl.AtlEProblemType problemType) {
	...
	protected void handleException(org.eclipse.core.runtime.CoreException ce) {
		if (ce.getMessage().matches("Marker.*not found.")) {
			...
			new atl.resource.atl.util.AtlRuntimeUtil().logError("Error while removing markers from resource:", ce);
		}
	}
		...
		COMMAND_QUEUE.addCommand(new atl.resource.atl.IAtlCommand<Object>() {
			public boolean execute(Object context) {
				try {
					...
					atl.resource.atl.mopp.AtlPlugin.logError("Can't create marker.", e);
					return false;
				}
			}
		});
