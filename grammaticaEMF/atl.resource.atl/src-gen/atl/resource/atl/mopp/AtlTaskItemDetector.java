...
/**
 * The AtlTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * AtlTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class AtlTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<atl.resource.atl.mopp.AtlTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<atl.resource.atl.mopp.AtlTaskItem> foundItems = new java.util.ArrayList<atl.resource.atl.mopp.AtlTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new atl.resource.atl.mopp.AtlTaskItem(keyword, message, localLine, offset, end));
