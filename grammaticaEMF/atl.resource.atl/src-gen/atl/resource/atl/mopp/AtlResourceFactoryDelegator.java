...
public class AtlResourceFactoryDelegator implements org.eclipse.emf.ecore.resource.Resource.Factory {
	
	protected java.util.Map<String, org.eclipse.emf.ecore.resource.Resource.Factory> factories = null;
	
	public AtlResourceFactoryDelegator() {
		init();
	}
	...
	protected void init() {
		...
		if (new atl.resource.atl.util.AtlRuntimeUtil().isEclipsePlatformAvailable()) {
			new atl.resource.atl.util.AtlEclipseProxy().getResourceFactoryExtensions(factories);
		}
		if (factories.get("") == null) {
			factories.put("", new atl.resource.atl.mopp.AtlResourceFactory());
		}
	}
