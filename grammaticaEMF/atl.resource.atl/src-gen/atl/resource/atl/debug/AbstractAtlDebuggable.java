...
public abstract class AbstractAtlDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<atl.resource.atl.util.AtlPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<atl.resource.atl.util.AtlPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new atl.resource.atl.util.AtlRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new atl.resource.atl.util.AtlRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(atl.resource.atl.debug.EAtlDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			atl.resource.atl.debug.AtlDebugMessage message = new atl.resource.atl.debug.AtlDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				atl.resource.atl.util.AtlPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(atl.resource.atl.debug.EAtlDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(atl.resource.atl.debug.EAtlDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new atl.resource.atl.util.AtlPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new atl.resource.atl.util.AtlPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(atl.resource.atl.debug.EAtlDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(atl.resource.atl.debug.EAtlDebugMessageTypes.STARTED, true);
			setSuspend(true);
