...
public class AtlDebugTarget extends atl.resource.atl.debug.AtlDebugElement implements org.eclipse.debug.core.model.IDebugTarget, atl.resource.atl.debug.IAtlDebugEventListener {
	
	/**
	 * associated system process
	 */
	private atl.resource.atl.debug.AtlDebugProcess process;
	...
	private atl.resource.atl.debug.AtlDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<atl.resource.atl.debug.IAtlDebugEventListener> eventListeners = new java.util.ArrayList<atl.resource.atl.debug.IAtlDebugEventListener>();
	
	private atl.resource.atl.debug.AtlDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new atl.resource.atl.mopp.AtlMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				atl.resource.atl.debug.AtlDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(atl.resource.atl.debug.AtlDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((atl.resource.atl.debug.IAtlDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public AtlDebugTarget(atl.resource.atl.debug.AtlDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new atl.resource.atl.debug.AtlDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new atl.resource.atl.debug.AtlDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(atl.resource.atl.debug.IAtlDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(atl.resource.atl.debug.IAtlDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new atl.resource.atl.mopp.AtlMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					atl.resource.atl.debug.AtlLineBreakpoint lineBreakpoint = (atl.resource.atl.debug.AtlLineBreakpoint) breakpoint;
	...
	public void handleMessage(atl.resource.atl.debug.AtlDebugMessage message) {
		try {
			if (message.hasType(atl.resource.atl.debug.EAtlDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(atl.resource.atl.debug.EAtlDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(atl.resource.atl.debug.EAtlDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(atl.resource.atl.debug.EAtlDebugMessageTypes.RESUMED)) {
	...
	public atl.resource.atl.debug.AtlDebugProxy getDebugProxy() {
