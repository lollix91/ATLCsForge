 ...
 */
public class AtlDebuggableInterpreter<ResultType, ContextType> extends atl.resource.atl.debug.AbstractAtlDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private atl.resource.atl.util.AbstractAtlInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private atl.resource.atl.IAtlCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public AtlDebuggableInterpreter(atl.resource.atl.util.AbstractAtlInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new atl.resource.atl.IAtlInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = atl.resource.atl.util.AtlStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public atl.resource.atl.util.AbstractAtlInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		atl.resource.atl.IAtlLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private atl.resource.atl.IAtlLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		atl.resource.atl.IAtlCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(atl.resource.atl.debug.EAtlDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = atl.resource.atl.util.AtlEObjectUtil.getDepth(current);
		stopCondition = new atl.resource.atl.IAtlCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = atl.resource.atl.util.AtlEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = atl.resource.atl.util.AtlEObjectUtil.getDepth(current) - 1;
		stopCondition = new atl.resource.atl.IAtlCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
