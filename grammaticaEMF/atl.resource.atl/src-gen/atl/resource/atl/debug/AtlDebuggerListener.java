 ...
 * code).
 */
public class AtlDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private atl.resource.atl.debug.AbstractAtlDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, atl.resource.atl.util.AtlPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, atl.resource.atl.util.AtlPair<String, Object>>();
	...
	public AtlDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		atl.resource.atl.debug.AtlDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(atl.resource.atl.debug.EAtlDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(atl.resource.atl.debug.EAtlDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(atl.resource.atl.debug.EAtlDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(atl.resource.atl.debug.EAtlDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(atl.resource.atl.debug.EAtlDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(atl.resource.atl.debug.EAtlDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(atl.resource.atl.debug.EAtlDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(atl.resource.atl.debug.EAtlDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = atl.resource.atl.util.AtlStringUtil.encode('#', stack);
				atl.resource.atl.debug.AtlDebugMessage message = new atl.resource.atl.debug.AtlDebugMessage(atl.resource.atl.debug.EAtlDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(atl.resource.atl.debug.EAtlDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				atl.resource.atl.debug.AtlDebugMessage message = new atl.resource.atl.debug.AtlDebugMessage(atl.resource.atl.debug.EAtlDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(atl.resource.atl.debug.EAtlDebugMessageTypes.GET_VARIABLES)) {
				...
				atl.resource.atl.debug.AtlDebugMessage message = new atl.resource.atl.debug.AtlDebugMessage(atl.resource.atl.debug.EAtlDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return atl.resource.atl.util.AtlStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		atl.resource.atl.util.AtlPair<String, Object> pair = new atl.resource.atl.util.AtlPair<String, Object>(name, value);
	...
	public atl.resource.atl.debug.AbstractAtlDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(atl.resource.atl.debug.AbstractAtlDebuggable debuggable) {
