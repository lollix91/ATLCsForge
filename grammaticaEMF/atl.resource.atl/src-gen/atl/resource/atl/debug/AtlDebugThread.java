...
public class AtlDebugThread extends atl.resource.atl.debug.AtlDebugElement implements org.eclipse.debug.core.model.IThread, atl.resource.atl.debug.IAtlDebugEventListener {
	
	private boolean suspended = false;
	private atl.resource.atl.debug.AtlDebugTarget debugTarget;
	
	public AtlDebugThread(atl.resource.atl.debug.AtlDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			atl.resource.atl.debug.AtlDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = atl.resource.atl.util.AtlStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new atl.resource.atl.debug.AtlStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public atl.resource.atl.debug.AtlDebugTarget getTarget() {
		return debugTarget;
	}
